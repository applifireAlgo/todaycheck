...
echo $PATH
OSNAME=`uname -s`
ART_DATA_PATH=/tmp/applifire/db/XCUE2MXC3AD7APQYXIBQRG/61B95991-97C3-4D61-A0CD-9EE7DF872F51/art/data
AST_DATA_PATH=/tmp/applifire/db/XCUE2MXC3AD7APQYXIBQRG/61B95991-97C3-4D61-A0CD-9EE7DF872F51/ast/data
MYSQL=/usr/bin
...
echo 'load AST data starts....'
$MYSQL/mysql --local-infile=1 -h$HOST -p$PORT -u$USER -p$PASSWORD $DB_NAME -e "load data local infile '$AST_DATA_PATH/ast_AppMenus_M.csv' into table ast_AppMenus_M FIELDS TERMINATED BY '#appfire#' LINES TERMINATED BY '\n' (menuId,menuTreeId,menuIcon,menuAction,menuCommands,menuDisplay,menuHead,menuLabel,uiType,RefObjectId,menuAccessRights,appType,appId,updatedBy,updatedDate,createdBy,createdDate,versionId,activeStatus,txnAccessCode)";
