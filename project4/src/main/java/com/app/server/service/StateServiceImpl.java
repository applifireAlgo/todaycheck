...
@SourceCodeAuthorClass(createdBy = "rushikesh.jeware@algorhythm.co.in", updatedBy = "", versionNumber = "1", comments = "Service for State Master table Entity", complexity = Complexity.LOW)
...
public class StateServiceImpl extends StateService {
    ...
    public HttpEntity<ResponseBean> findById(@RequestBody FindByBean findByBean) throws SpartanPersistenceException, Exception {
        ...
        org.springframework.http.HttpStatus httpStatus = org.springframework.http.HttpStatus.OK;
        try {
            com.app.shared.location.State lststate = staterepo.findById((java.lang.String) findByBean.getFindKey());
            responseBean.add("success", true);
            responseBean.add("message", "Successfully retrived ");
            responseBean.add("data", lststate);
        } catch (org.springframework.transaction.TransactionException e) {
